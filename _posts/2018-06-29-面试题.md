...
-  **更多高效分页查询：** https://blog.csdn.net/honey_potter/article/details/53014284 （ORACLE分页查询SQL语法——最高效的分页）
...
- **右连接：**恰与左连接相反，返回右表中的所有行，如果右表中行在左表中没有匹配行，则结果中左表中的列返回空值。 **右连接恰与左连接相反，显示右表全部行，和左表与右表相同行。** 
  ...
  **更多内容详见：** https://blog.csdn.net/jiuqiyuliang/article/details/10474221 详解SQL Server连接（内连接、外连接、交叉连接）
...
3. **响应正文**：服务器回传数据。
   ...
   **更多详细内容：** https://blog.csdn.net/chen1403876161/article/details/51546653（ Http请求和Http响应详细解析） 
...
4.  安全的和幂等的。所谓安全的意味着该操作用于获取信息而非修改信息。幂等的意味着对同一 URL的多个请求应该返回同样的结果。完整的定义并不像看起来那样严格。换句话说，GET请求一般不应产生副作用。从根本上讲，其目标是当用户打开一个链接时，她可以确信从自身的角度来看没有改变资源。比如，新闻站点的头版不断更新。虽然第二次请求会返回不同的一批新闻，该操作仍然被认为是安全的和幂等的，因为它总是返回当前的新闻。反之亦然。POST请求就不那么轻松了。POST表示可能改变服务器上的资源的请求。仍然以新闻站点为例，读者对文章的注解应该通过 POST请求实现，因为在注解提交之后站点已经不同了。
   ...
   **原文链接：**  https://blog.csdn.net/lovestudy_girl/article/details/53095588（ HTTP请求方式GET和POST的区别详解） 
 ...
 **具体详见：**https://blog.csdn.net/ruangong1203/article/details/52577114（jquery事件绑定解绑机制源码分析）
